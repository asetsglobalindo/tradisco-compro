    ...
    <section id="select-input" className='flex flex-col mb-4 mx-4'>
      ...
      {({ open }) => (
        <section>
          ...
          <ListboxOptions anchor="bottom" className="border border-neutral-400 w-[368px] max-h-[192px] text-left bg-white">
            {collaborationType.map((type) => (
              <ListboxOption key={type.id} value={type} className="group flex gap-2 bg-white data-[focus]:bg-blue-100 data-[selected]:bg-green-light data-[selected]:text-white p-4">
                {type.name}
              </ListboxOption>
            ))}
          </ListboxOptions>
