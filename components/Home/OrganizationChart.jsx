...
function nodeHTML(d) {
  ...
  const desc = d.data.description
    ? `<div class="text-[11px] text-gray-600 text-center mt-1 line-clamp-3 px-1">${d.data.description}</div>`
    : "";
  return `
    <div class="node-card w-full h-full p-2 rounded-lg bg-white border shadow-sm" style="box-shadow:0 6px 18px rgba(15,23,42,0.06);">
...
const OrganizationChartD3 = () => {
  ...
  useEffect(() => {
    ...
    // Group for pan/zoom
    const g = svg.append("g").attr("class", "layer-root").attr("transform", `translate(40, ${height / 2})`)    // start center vertically
    ...
    const nodeHeight = 110; // height of card
    const horizontalSpacing = 260; // space between levels
    ...
    function update(source) {
      ...
      nodeMerge.select("foreignObject.fo").html((d) => nodeHTML(d));
    ...
    // Center initial view: translate so root is at left margin
    const initialX = 20;
    const initialY = 0;
    svg.transition().duration(300)
  .call(zoom.transform, d3.zoomIdentity.translate(40, height / 2).scale(0.95));
    ...
    const ro = new ResizeObserver(() => {
      ...
      // reposition root group for vertical centering
      g.attr("transform", `translate(20, ${newHeight / 2})`);
      // recompute layout
      update(root);
    });
  ...
  return (
    <section className="py-8 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div ref={containerRef} className="w-full h-[520px] border rounded-lg bg-gray-50 overflow-hidden">
          <svg ref={svgRef} />
        </div>
      </div>
    </section>
